...
export class ExplosionParticle extends Sphere implements Entity {
  ...
  public update(): void{
    this.location.x -= this.direction.x * (this.speed * (0.02 + Math.random()*0.5));
    this.location.y -= this.direction.y * (this.speed * (0.02 + Math.random()*0.5));
    this.location.z += this.direction.z * (this.speed * (0.02 + Math.random()*0.5));
    if(this.radius > 0)
